# Contributing to Arkan Help — المساهمة في Arkan Help
...
```bash
git clone https://github.com/YOUR_USERNAME/arkan_help.git
cd arkan_help
```
...
```bash
bench get-app arkan_help /path/to/your/fork
bench --site dev.localhost install-app arkan_help
...
bench --site dev.localhost run-tests --app arkan_help

# Run specific test — تشغيل اختبار محدد
bench --site dev.localhost run-tests --app arkan_help --module arkan_help.tests.unit.test_my_service
```
...
- Update help files in `arkan_help/help/` for DocType changes
