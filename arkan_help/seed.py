...
def seed_data():
    ...
    """
    frappe.logger().info("Arkan Help: Running seed_data()...")

    _seed_settings()
    _seed_roles()

    frappe.db.commit()
    frappe.logger().info("Arkan Help: seed_data() complete.")
...
def _seed_settings():
    ...
    settings_dt = None
    for dt_name in ["AH Settings", "Arkan Help Settings", "Arkan_Help Settings"]:
...
def _seed_roles():
    ...
    roles = [
        {"role": "AH User", "desk_access": 1},
        {"role": "AH Manager", "desk_access": 1},
        {"role": "AH Admin", "desk_access": 1},
    ]
