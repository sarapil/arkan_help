"""
CAPS Permission Gate for Arkan Help
...
def has_capability(user=None, capability=None):
    """Check if user has a specific Arkan Help capability via CAPS."""
    if not user:
    ...
    if not capability.startswith("AH_"):
        capability = f"AH_{capability}"
